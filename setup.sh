...
PACMAN_PKGS=(
  ...
  grim wl-clipboard cliphist wf-recorder slurp
  # Utilities
  jq python python-pip playerctl brightnessctl
  pacman-contrib cpupower imagemagick bc fastfetch tmux
)
...
if [ "$IS_ARCH" = true ]; then
  ...
  # ---- 3a. Install NVIDIA drivers if NVIDIA GPU detected ----
  if lspci 2>/dev/null | grep -qi "nvidia" && ! pacman -Qi nvidia-dkms &>/dev/null; then
    info "NVIDIA GPU detected — installing drivers ..."
    sudo pacman -S --needed nvidia-dkms nvidia-utils nvidia-settings || \
      warn "NVIDIA driver install failed — install manually: sudo pacman -S nvidia-dkms nvidia-utils"
    ok "NVIDIA drivers installed"
  fi

  # ---- 3b. Enable services ----
  for svc in pipewire wireplumber pipewire-pulse bluetooth NetworkManager; do
...
# ===========================================================================
#  12. Offer to start Hyprland now
# ===========================================================================
echo -e "${YELLOW}  ┌─────────────────────────────────────────────┐${NC}"
echo -e "${YELLOW}  │  Запустить Hyprland сейчас?                   │${NC}"
echo -e "${YELLOW}  │  Введи YES (да) или что угодно для отмены     │${NC}"
echo -e "${YELLOW}  └─────────────────────────────────────────────┘${NC}"
echo ""
read -r -p "> " ANSWER
echo ""
...
if [ "$ANSWER" = "YES" ]; then
  ...
  echo -e "${GREEN}  Если это первый запуск — закрой окно предупреждения${NC}"
  echo -e "${GREEN}  через Super+Q или Alt+F4, и всё подгрузится${NC}"
  sleep 2
  exec Hyprland
else
  echo ""
  echo "  Запустить позже:"
  echo "       Hyprland"
  echo ""
  echo "  Если цвета не применились:"
